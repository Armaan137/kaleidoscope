...
std::unique_ptr<llvm::LLVMContext> context;      // Object that has core LLVM data structures. Stored unique values.
std::unique_ptr<llvm::IRBuilder<>> builder;      // Helper object that helps generate LLVM instructions. Instances of IRBuilder class template keeps track of the current place to insert and has methods to create new instructions. 
std::unique_ptr<llvm::Module> module;            // Construct that contains functions and global variables. Will own the memory for all the IR generated.
std::map<std::string, llvm::Value*> namedValues; // Keeps track of which values are defined in the current scope and what their LLVM representation is; symbol table.
